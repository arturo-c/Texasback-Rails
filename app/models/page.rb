class Page < ActiveRecord::Base
  ...
  validates_uniqueness_of :slug, :message => 'is already in use.  Please enter a different URL.'
  ...
  # Added for texas back
  def update_content(options)    
    # Update the body content first
    if update_part_content('body', options[:body])
      # update the title_tag
      if update_part_content('title_tag', options[:page_title])
        # then update the description/keywords      
        self.description = options[:description]
        self.keywords = options[:keywords]
        
        if options[:status_id]
          self.status_id = options[:status_id]
        end
        
        update_part_content('secondary', options[:secondary]) if options[:secondary]
  ...
  # updates content for a particular page part, and adds it if it doesn't exist
  def update_part_content(part_name, content)
    the_part = self.part(part_name.to_s)
    
    unless the_part
      the_part = PagePart.new :page_id => self.id, :name => part_name.to_s
    end
    
    the_part.content = content.to_s
    
    the_part.save
  end
  
  def part_content(name)
    the_part = self.part(name.to_s)
